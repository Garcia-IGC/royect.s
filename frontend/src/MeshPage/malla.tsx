...
const Malla: React.FC<MallaProps> = ({ userData }) => {
    ...
    return (
                          ...
                          <div className="space-y-2">
                            {asignaturas.map((asig, i) => {
                              ...
                              return (
                                <div
                                  key={i}
                                  className="relative bg-gradient-to-br from-teal-50 to-cyan-50 border-l-4 border-teal-400 rounded-lg p-2.5 shadow-md hover:shadow-lg transition-all duration-200 hover:scale-105 cursor-pointer"
        ...
        {view === 'cursados' && (
          <>
            {data.carreras.map((carrera, idx) => {
              const cursados = carrera.malla.filter((a) => a.cursada);
              return (
                <div key={idx} className="mb-8">
                  <div className="bg-gradient-to-r from-indigo-600 to-blue-500 rounded-t-xl p-3 shadow-lg">
                    <h2 className="text-xl font-bold text-white">{carrera.carrera}</h2>
        ...
        {/* Vista: Proyección (pendientes) */}
        {view === 'proyeccion' && (
          <>
            {data.carreras.map((carrera, idx) => {
              const pendientes = carrera.malla.filter((a) => !a.cursada);
              return (
                <div key={idx} className="mb-8">
                  <div className="bg-gradient-to-r from-indigo-600 to-blue-500 rounded-t-xl p-3 shadow-lg">
                    <h2 className="text-xl font-bold text-white">{carrera.carrera}</h2>
